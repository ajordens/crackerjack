...
class SessionAuthInjectable extends AbstractHttpContextInjectable {
    static final Logger LOG = LoggerFactory.getLogger(SessionAuthInjectable.class);

    private final SessionHandler sessionHandler
    private final PersonDAO personDAO

    SessionAuthInjectable(SessionHandler sessionHandler, PersonDAO personDAO) {
    ...
    public User getValue(HttpContext c) {
        try {
            ...
            if (sessionId) {
                ...
                if (session) {
                    User currentUser = (User) session.getAttribute(AuthHelper.SESSION_USER)
                    if (currentUser) {
